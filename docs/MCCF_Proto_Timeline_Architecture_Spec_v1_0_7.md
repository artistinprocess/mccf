...
## Version 1.7 — Day 63/64 (§2.2 confirmed by test; PROTOs decided as required. All 22 camera shot types confirmed working. §5c resolved into a three-class camera model. v1.7 adds the sentiment-lint authoring aid to §5a, motivated by the Day 64 voice-agent experiment. Everything else remains design-only.)
...
- **`Zone_{id} enter` / `Zone_{id} exit`.** Correction, Day 63, verified against the actual code: the existing zone-audio detection is **not** `ProximitySensor` — confirmed impossible, `ProximitySensor` is viewer/camera-only per the X3D spec, and the codebase's own comment says so directly ("ProximitySensors detect the viewer camera, not avatars"). The real mechanism is `spPollAvatarZones()`/`spTriggerZone()` — JS-side polling of each avatar's translation, distance-checked against zone radius, already firing enter/exit for the ambient audio fade today. Generalizing into `EventCues` means `spTriggerZone` also calls `fireEventCuesForTrigger` — still no new detection mechanism needed, just the wrong one credited originally.
- **Camera/viewer-based proximity, a distinct and separate capability, genuinely free via native `ProximitySensor`**: `isActive`/`enterTime`/`exitTime` for discrete enter/exit, `position_changed`/`orientation_changed` for continuous data while inside — real eventOuts, confirmed against spec, usable for camera-region triggers or as a Follower/Chaser source per §2's parenting discussion. Not a substitute for avatar zone detection; a different, additional thing.
- **Debug technique, worth keeping**: `CameraTestAll_scene.xml`'s pattern — add spoken dialog per waypoint, set `dwell` long (e.g. 10s) — turns the test file into a general event-system debugger, not just a camera one. Long dwell = a budget for how many cues can meaningfully fire and be observed before the avatar moves on.
...
### Sentiment lint pass — future dialog-editor authoring aid (Day 64)

Motivated by a Day 64 experiment: the MCCF Voice Agent UI (`mccf_voice.html`), pointed at a local Ollama model, proved out live persona-shaped dialog with a visible affect-state panel — effectively the first live exercise of the `emotionalInterpreter` concept, even at painful latency. Two features of that UI are worth pulling forward into the dialog editor when it's built: live channel sliders for setting `channelE/B/P/S` per line (drag, don't type floats), and a per-line **audition** button (the Test TTS pattern) that closes the write→hear loop without a full scene export. The voice params block (rate/pitch/volume/pause) maps onto per-line TTS overrides.

The lint pass itself: feed a `DialogLine`'s `text` to a sentiment/affect analyzer, get back a *predicted* `ChannelVector`, and diff it against the line's *authored* `channelE/B/P/S`. Large divergence — a line authored high-arousal/negative-valence that the analyzer reads as flat — gets **flagged, never auto-corrected**, the same philosophy as the registry's text-hash drift detection. It's a lint pass for emotional authoring, not an authority.

Three constraints, decided now so they aren't re-litigated later:
- **Authoring-time only.** The analyzer suggests; the author decides; the authored `ChannelVector` is what ships. This must not become a runtime recomputation path — the server stays authoritative on computed affect (§2.7), and the client/renderer never recomputes semantic values.
- **Right-sized for a slow local model.** Sentiment classification is forgiving in exactly the way X3D syntax generation is not: approximate reads are still useful, there is no silent-failure mode, and it can run as a batch/background pass over a scene's dialog track where multi-minute latency costs nothing. The Day 64 experiment showed the local model is confidently wrong on spec-precision tasks (it emitted an `SFInt32` field holding `0.75`) — disqualifying for `EXTERNPROTO` generation, acceptable for "does this line feel like what you tagged it as."
- **Same vector, no parallel representation.** Predicted affect is expressed as the same `ChannelVector` (E/B/P/S) shape as everything else in this spec — the diff is vector-to-vector, plottable on the same dashboard as live agent state.

**Status: designed, not built.** Belongs to the dialog editor (separate tool, shared contract, above), not the Events Editor.
...
| 15 | Sentiment lint pass (predicted vs authored `ChannelVector` diff) + channel sliders + per-line audition, for the dialog editor | §5a | Designed Day 64, not built |
...
*Day 64 (v1.7): Sentiment lint pass added to §5a as a future dialog-editor authoring aid, motivated by the voice-agent/Ollama experiment — which also served as the first live exercise of the `emotionalInterpreter` concept and demonstrated the local model's fitness boundary (usable for forgiving batch affect analysis; not trustworthy for spec-precision X3D generation). Channel sliders and per-line audition noted as UI borrows from `mccf_voice.html`. Version header note: the v1.6 file's header still read "Version 1.5" — header and filename are now reconciled at 1.7.*
