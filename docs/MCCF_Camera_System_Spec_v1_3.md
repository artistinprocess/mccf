...
> **Day 78 status note — read this before the rest of the document.**
> `MCCF_Camera_Model_Revision_v0.1.md` (Day 77) replaced this spec's core
> organizing principle. The new question is *"does this camera's position
> ever depend on a subject's live position at cue-fire time"* — not *"what
> shot type is this."* Concretely, **§1–§3 and §7's static/relational
> shot-type taxonomy (Wide/Medium/Closeup/Over-Shoulder/Two-Shot/Profile/
> etc.) are superseded** — those become placed cameras authored in the
> Place grid, not live per-cue formulas. §3a's agent-attached/parented
> pattern **survives, generalized**: `agent_orbit` and `agent_track` join
> `agent_eye`/`agent_side` as fully attached types once the
> Character-Creator/Events split lands (Camera Model Revision §1).
>
> **A full v1.4 rewrite reflecting this has not been done** — that's real
> design-transcription work (DEF naming/seeding rules, roll composition,
> the new Place Editor shell) that deserves the author's own review rather
> than a rushed condensation. Treat `MCCF_Camera_Model_Revision_v0.1.md`
> §1–§4 as authoritative over this document's §1–§3/§7 until that rewrite
> happens.
>
> **What's still accurate and load-bearing as-is:** §4 (X_ITE SAI API
> corrections — extended below with a new Day 78 finding), §5's
> orientation math, §6 (`VP_Free` vessel pattern), §10 (Loader routing
> rules, still structurally correct even where shot types feeding into it
> have changed). Two small corrections applied inline below, both already
> flagged elsewhere as needed: §9's `track` status, and §3a's "generic
> across every avatar" language (per
> `MCCF_Avatar_Camera_Rig_Manifest_Spec_v0.1.md`'s explicit correction
> instruction).
...
These three Viewpoints (`Eye`, `Side`, `Orbit`) were **generic and identical
across every avatar** through Day 75 — added uniformly by the Composer's
`buildAvatarX3D` function regardless of which character was being exported,
not part of any individual avatar's authored asset. **Superseded Day 76**
(`MCCF_Avatar_Camera_Rig_Manifest_Spec_v0.1.md` §4): this generic
scene-export-time injection is now rejected outright — it polluted the
X_ITE viewpoint menu with near-duplicate generic cameras across every
avatar in a scene. Character Creator is now the only source of per-avatar
camera rigs; an avatar with no Character-Creator-authored rig has no
camera options offered by the Events Editor at all, rather than falling
back to this generic set. See that spec for the full manifest/discovery
...
### A bound Viewpoint's `position`/`orientation` fields do NOT track live navigation — confirmed Day 78

Discovered while building the path recorder (`MCCF_V5_Integration_Nominal_Order_and_Tasklist_v0_1.md` Phase 2), but belongs here: it's a camera/Viewpoint SAI fact, not a path-recorder-specific one, and the wrong assumption could resurface anywhere that reads a Viewpoint's live pose.

**Wrong assumption, initially built on real (but misapplied) precedent:** `getActiveViewpoint().getField('position'/'orientation').toString()`, polled on a timer, was assumed to reflect the live camera pose as the user navigates (mouse-drag, fly/walk). This looked well-founded — `updateCamTelemetry` elsewhere in the Loader already uses exactly this read pattern successfully — but that existing code was always reading back a value *the Loader itself had just written via script* (`CAM_Free_Transform`, direct SAI assignment), never reading live user-driven navigation. Those are different claims, and only the first one had real precedent.

**Confirmed wrong, by direct test, not by assumption:**
```javascript
// Run once, note the value. Navigate the viewport (mouse-drag/fly).
// Run again with the SAME Viewpoint still bound — the value does NOT change,
// even though the viewport visibly moved:
document.getElementById('vp-x3d').browser.getActiveViewpoint().getField('position').toString()
// Rebinding to a DIFFERENT named Viewpoint, however, DOES change the value —
// confirming this reads the node's authored position, not a live transform.
```

**Confirmed independently against X_ITE's own documentation**, not just this one empirical test — two separate official sources:
- ["Controlling the Viewpoint"](https://create3000.github.io/x_ite/tutorials/controlling-the-viewpoint/): navigation with `jump=false` preserves "the previous bound viewpoint... **including any user navigation offsets**" when rebinding — confirming navigation is tracked as an offset layered on top of a Viewpoint's own fields, never written into them.
- ["Sensing the Viewer"](https://create3000.github.io/x_ite/tutorials/sensing-the-viewer/), stated as the actual documented mechanism, not a fallback: *"A ProximitySensor node senses the viewer while in a region: position and orientation — sends position and orientation while the viewer is in the region."*

**The correct mechanism**: a `ProximitySensor` sized to cover the navigable volume, reading its `position_changed`/`orientation_changed` eventOut fields (same polling pattern as above — only *what* is read changes, not the mechanism). Confirmed live: a deliberately-flown curve went from collapsing to 2 keyframes (reading the wrong, static Viewpoint field) to 39 real keyframes (reading the sensor correctly) with identical downstream RDP decimation code — the fix was entirely about which node to read, not the read mechanism itself.

**Practical implication for any future camera work that needs the live navigated pose** (not just path recording): reach for `ProximitySensor.position_changed`/`orientation_changed`, sized to the volume that needs sensing, never a bound Viewpoint's own fields — regardless of read mechanism (polling or `addFieldCallback`, both would fail identically here, since the field itself is never updated by navigation). See `MCCF_V5_Integration_Nominal_Order_and_Tasklist_v0_1.md`'s Day 78 corrections section for the full path-recorder implementation this produced.
...
| `track` agent-attached conversion | **Corrected Day 78** (was: "Best remaining candidate after `over_shoulder`/`profile`" — see original note below): `AgentTrackCamera` already exists in `mccf_camera_protos.x3d` as of Day 64, per `MCCF_Avatar_Camera_Rig_Manifest_Spec_v0.1.md` §4's explicit correction. This was never a future candidate — the proto was already built. Original Day 62 framing, kept for context: genuinely needs to follow a moving subject from the side. |
...
*Day 78. Status banner added at top: core shot-type taxonomy (§1-§3, §7) superseded by `MCCF_Camera_Model_Revision_v0.1.md`'s placed-vs-parented model — full v1.4 rewrite not yet done, deliberately, pending author review of the design-transcription work that requires. Two small corrections applied: §3a's "generic across every avatar" language (superseded Day 76, per Avatar Camera Rig Manifest Spec) and §9's `track` status (the proto already existed as of Day 64, was never a future candidate). New §4 addition: a bound Viewpoint's position/orientation fields do NOT track live navigation, confirmed by direct test and against X_ITE's own docs — ProximitySensor.position_changed/orientation_changed is the correct mechanism, not a fallback. Full story and the path-recorder implementation this produced: `MCCF_V5_Integration_Nominal_Order_and_Tasklist_v0_1.md`'s Day 78 section.*
