...
## Version 1.2 — Day 60 (VP_Free pattern confirmed; coordinate system)
...
> **Architectural note — Baked vs Runtime cameras**
> See `MCCF_Events_Editor_Architecture.md` for the full system-wide treatment.
> The camera system has two fundamentally different resolution paths that must never be mixed:
>
> **Baked cameras** are Viewpoint nodes the X3D author explicitly placed in the scene file.
> The Loader binds them by name via `set_bind`. No position computation. No VP_Free.
> — `agent_eye` / `agent_side`: VP inside the agent's own Transform, rides with the avatar.
> — Named `viewpoint=` in cue: any VP the author placed and referenced by name.
>
> **Runtime cameras** are computed by the Loader at playback time and applied to the
> `VP_Free` vessel node — a generic Viewpoint emitted by the Composer into every scene.
> VP_Free has no meaningful authored position. The author never names it in a cue.
> — Static shot types (`wide`, `medium`, etc.): Loader computes position from cue params.
> — Move shot types (`orbit`, `dolly_in`, etc.): Loader drives VP_Free via TimeSensor (Phase 2).
>
> VP_Free is never bound as a fallback or default. It only fires when the author
> explicitly chose a static or move shot type from the Events Editor.

### Step 1 — Identify shot category
- If `shot` is `agent_eye` or `agent_side` → **baked agent-attached path**
- If `cue.viewpoint` is set → **baked named VP path**
- If `shot` is a static type (`wide`, `medium`, `closeup`, etc.) → **runtime static path**
- If `shot` is a move type (`dolly_in`, `orbit`, etc.) → **runtime interpolated path** (Phase 2)
- Otherwise → **warn and skip — no fallback**
...
**Rules for VP_Free:**
- The Composer emits it into every exported X3D scene automatically, wrapped in `CAM_Free_Transform`.
...
- On Stop/Reset, VP_Free's bind state is cleared alongside all other runtime state.

**Implementation pattern (confirmed Day 60):**

X3D Viewpoint `position` and `orientation` are output fields — SAI writes to them are ignored.
The Loader writes to `CAM_Free_Transform` (the parent Transform), then binds VP_Free:

```javascript
// CORRECT — write to Transform parent, bind Viewpoint
...
// WRONG — Viewpoint position/orientation are output-only, writes ignored
...
The `_fireCameraEventCue` function uses two explicit whitelists to enforce the baked/runtime boundary:

```javascript
// Baked cameras — Loader binds a named authored node
// 1. agent_eye / agent_side  →  _bindNamedViewpoint('VP_{subject}_{Eye|Side}')
// 2. cue.viewpoint set       →  _bindNamedViewpoint(cue.viewpoint)

// Runtime cameras — Loader drives VP_Free vessel via SAI
// 3. _STATIC_SHOTS[shot]     →  _executeStaticCameraShot(cue)
// 4. _MOVE_SHOTS[shot]       →  _executeMoveShot(cue)  [Phase 2]
...
`_bindNamedViewpoint` and `_executeStaticCameraShot` / `_executeMoveShot` are strictly separated — they never cross-call. Adding a new shot type requires adding it to the appropriate whitelist; the routing logic itself does not change.
