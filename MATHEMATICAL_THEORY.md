...
**Ground truth:** mccf\_core.py v1.5.0, mccf\_hotHouse.py v2.2, mccf\_collapse.py v1.7, mccf\_voice\_api.py v2.3
...
## 12b\. Hybrid Discrete/Continuous Architecture

*Acknowledgment added in response to Kate review, April 2026.*

MATHEMATICAL\_THEORY.md describes MCCF as a smooth dynamical system with continuous
ODEs and differentiable operators. This is accurate at the field mechanics level.
However the full implementation is a **hybrid discrete/continuous system** and this
distinction should be made explicit:

**Continuous components:**
- Euler integration of $d\\psi/dt$ in `mccf\_hotHouse.py`
- TrustField dynamics (β, γ differential equations)
- CCS power blend (smooth, differentiable for all raw > 0)
- Damping term (continuous friction proportional to deviation)

**Discrete/threshold components:**
- MetaState mode classification (exploit/explore/repair/avoid — threshold-gated)
- Asymmetry classification (benign/unstable/pathological — threshold-gated)
- Echo chamber risk flags (ECHO\_HIGH, ASYMMETRIC, PARASOCIAL — threshold-gated)
- tanh decomposition matrix (smooth but threshold-anchored at THRESHOLD=2)
- TrustField hysteresis (binary rupture flag — discrete state transition)
- Genre classification (comedy/drama/tragedy — rule-based on coherence metrics)

The system is correctly described as a **piecewise smooth dynamical system** with
discrete classification layers operating on top of continuous field mechanics.
The continuous Hamiltonian governs field evolution; the discrete classifiers
provide observable categorical outputs for analysis and narrative use.

This hybrid architecture is intentional. The continuous layer provides physical
grounding. The discrete layer provides legible outputs. Future formalization
should treat these as separate but coupled subsystems.
...
At $\\sigma=1.0$: $\\alpha=1.0$, modulated $= R^{\\text{raw}}$ (full signal fidelity).  
At $\\sigma=0.2$: $\\alpha=1.8$, modulated $= (R^{\\text{raw}})^{1.8}$ (proportional attenuation).

The previous compressed blend $\\sigma \\cdot R + (1-\\sigma) \\cdot R^2$ over-penalized weak signals
— at $R=0.2, \\sigma=0.2$: modulated $= 0.072$, nearly noise-level. The power blend gives
$0.2^{1.8} = 0.055$ — attenuated proportionally, not collapsed.
