...
def build_affective_system_prompt(
    ...
    # v2.0 — Δ trajectory from agent delta_history
    # Passed in from voice_speak after computing agent.delta_context()
    delta_ctx = delta_context or affective_context.get("delta_context", {})
...
"""
    ...
    # v2.0 — Δ trajectory injection
    if delta_ctx and delta_ctx.get("window", 0) > 0:
        trend      = delta_ctx.get("trend", "stable")
        trajectory = delta_ctx.get("trajectory", "")
        mean_delta = delta_ctx.get("mean", 0.0)
        prompt += (
            f"YOUR EMOTIONAL TRAJECTORY (last {delta_ctx['window']} interactions):\n"
            f"  Trend: {trend} (mean delta: {mean_delta:+.3f})\n"
            f"  {trajectory}\n\n"
        )
...
class LLMAdapter(ABC):
    ...
    def _inject_affective_context(
        ...
        v2.0: passes delta_context to build_affective_system_prompt
        for emotional trajectory injection.
        """
...
class StubAdapter(LLMAdapter):
    ...
    async def complete(self, messages, affective_context, persona,
                       params=None, delta_context: dict = None) -> AsyncIterator[str]:
...
class AnthropicAdapter(LLMAdapter):
    ...
    async def complete(self, messages, affective_context, persona,
        ...
        p = self._default_params(params)
        injected = self._inject_affective_context(messages, affective_context, persona, delta_context=delta_context)
...
class OllamaAdapter(LLMAdapter):
    ...
    async def complete(self, messages, affective_context, persona,
        ...
        # Synchronous urllib call — avoids Windows asyncio/aiohttp issues
        content = ""
        # v2.0 — Increased timeout + retry for constitutional arc waypoints.
        # Waypoint prompts are longer than conversational turns.
        # Timeout ladder: try 180s first, retry once at 240s on timeout.
        TIMEOUTS = [180, 240]
        last_error = None
        ...
        if last_error:
            if "timed out" in str(last_error).lower() or "timeout" in str(last_error).lower():
                content = (
                    ...
                    "Try a shorter waypoint prompt, or restart Ollama. "
                    f"Last timeout was {TIMEOUTS[-1]}s.]"
                )
            else:
                content = f"[Ollama error: {last_error}]"
        ...
        if content:
            ...
            yield "[Ollama: no content returned]"
