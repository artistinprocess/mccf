...
def make_shibboleth_api(field, honor_constraint, api_url="http://localhost:5000"):
    ...
    def run_test(agent_name):
        ...
        # v2.0 — asyncio.run() is safer on Python 3.12+ Windows
        try:
            report = asyncio.run(test_runner.run_full_test(agent_name))
        except RuntimeError:
            # Fallback if event loop already running (e.g. Jupyter)
            loop = asyncio.new_event_loop()
            try:
                report = loop.run_until_complete(test_runner.run_full_test(agent_name))
            finally:
                loop.close()
    ...
    def run_single_probe(agent_name, probe_id):
        ...
        # v2.0 — asyncio.run() is safer on Python 3.12+ Windows
        try:
            result = asyncio.run(test_runner.run_probe(agent_name, probe_id))
        except RuntimeError:
            loop = asyncio.new_event_loop()
            try:
                result = loop.run_until_complete(test_runner.run_probe(agent_name, probe_id))
            finally:
                loop.close()
