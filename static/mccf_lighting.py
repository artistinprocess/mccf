...
def compute_lighting(
    ...
    if zones:
        z0 = zones[0]
        # Safe extraction: always use string fallback, never the dict itself
        if isinstance(z0, dict):
            zone_type = str(z0.get("zone_type", "neutral"))
        elif isinstance(z0, str):
            zone_type = z0
        # Ensure zone_type is a valid preset key
        if zone_type not in ZONE_LIGHT_PRESETS:
            zone_type = "neutral"
    ...
    key_intensity = preset["key_intensity"] * (0.85 + E * 0.15)  # raised floor for scene brightness
    ...
    ambient_intensity = 0.30 + (1 - contrast) * 0.20  # raised floor for scene visibility
    ambient_rgb = kelvin_to_rgb(max(5000, kelvin + 1500))

    # Rim: behavioral channel — high B adds clean back edge
    B = zp.get("B", 0.0)
    rim_intensity = 0.18 + eng * 0.12 + B * 0.10
